...
function getOrder() {
	$j.getJSON('http://cosyhh.aliapp.com/api.php', function (data) {
		...
		if (data.message == 'success') {
			...
			if (v1 < v2) {
				$j('#rosd_h4').html('Real-time Order - <a href="http://cosyhh.aliapp.com/rosd/rosd.crx">A newer version is available!</a>');
			} else {
...
function add_rosd() {
	...
	$j('#info_a').click(function () {
		...
		$j.get('http://www.erepublik.com/en/military/campaigns', function (data) {
			var battle, attack, defend, region,
				tmp = data.split('full_width">')[1].split('</div>\n	\t<ul class="v')[0],
				up_list = $j('<div>' + tmp + '</div>'),
				s = [],
				postInfo = {upinfo: 0, info: ''};
			...
			postInfo.info = '[' + s.join(',') + ']';
			$j.post('http://cosyhh.aliapp.com/api.php', postInfo, function (data) {
				if (data.message == 'success') {
